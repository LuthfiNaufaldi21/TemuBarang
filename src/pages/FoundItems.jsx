...
export default function FoundItems() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await postsAPI.getAll({ type: "found" });
        ...
        <main className="flex-1 min-h-0 overflow-y-auto px-4 pt-4 pb-[calc(6.5rem+env(safe-area-inset-bottom))] md:p-8 md:pb-8 flex flex-col items-center">
          <div className="w-full max-w-300 mx-auto flex flex-col gap-6">
            <div className="flex flex-col gap-1 w-full">
              <h2 className="text-[#DDE4DD] text-3xl font-bold mb-1">Found Items Gallery</h2>
              <p className="text-[#A1A1AA] text-base">Browse items that have been found on campus.</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="w-full min-h-100 flex flex-col items-center justify-center bg-[#1A211D] border border-dashed border-[#4D774E]/50 rounded-2xl py-20 px-6 text-center mt-2">
                <p className="text-[#DDE4DD] text-lg font-semibold">No found items yet</p>
                <p className="text-[#86948A] text-sm mt-1">
                  {allPosts.length === 0 ? "No items have been reported as found yet." : "No items match the selected category."}
                </p>
              </div>
            ) : (
                ...
                {current.map((post) => (
                  <div key={post.post_id} className="bg-[#1A211D] border border-[#3C4A42]/30 rounded-xl overflow-hidden shadow-sm flex flex-col">
                    <div className="h-48 bg-[#2F3632] relative overflow-hidden flex items-center justify-center">
                      ...
                      <div className="absolute top-4 right-4 z-20 text-white text-[11px] font-bold px-3 py-1 rounded-full backdrop-blur-md border bg-[#11996C]/50 border-[#11996C]">FOUND</div>
                    </div>
            ...
            {filtered.length > 0 && (
              <div className="w-full flex flex-col md:flex-row justify-between items-center gap-4 mt-6 pt-5 pb-2 border-t border-[#27272A]">
                <div className="text-[#71717A] text-sm">Showing <span className="text-[#DDE4DD] font-semibold">{filtered.length}</span> found items</div>
                  {totalPages > 1 && (
