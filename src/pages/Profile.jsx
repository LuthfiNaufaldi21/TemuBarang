      ...
      <div className="flex-1 min-w-0 min-h-0 flex flex-col h-full overflow-hidden">
        ...
        <main className="flex-1 min-h-0 overflow-y-auto px-4 pt-4 pb-[calc(6.5rem+env(safe-area-inset-bottom))] md:p-8 md:pb-8 flex flex-col items-center">
          <div className="w-full max-w-6xl flex flex-col gap-8">
            <div className="relative w-full rounded-2xl overflow-hidden bg-[#18181B] shadow-md">
              ...
              <div className="relative p-5 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex flex-col md:flex-row items-center gap-6 w-full">
                  <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-[#9CC88D] bg-[#164A41] shadow-2xl flex items-center justify-center shrink-0 overflow-hidden">
                    {user?.avatar_url ? (
                      <img src={user.avatar_url} alt="Profile" className="w-full h-full object-cover" />
                    ) : (
                      <svg className="w-20 h-20 md:w-24 md:h-24 text-[#9CC88D] absolute -bottom-2" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left gap-1">
                    <h2 className="text-[#DDE4DD] text-2xl md:text-3xl font-bold break-words">{user?.full_name || "—"}</h2>
                    <div className="w-full flex flex-col md:flex-row md:flex-wrap items-center md:items-start gap-3 md:gap-4 mt-2">
                      {user?.student_id_number && (
                        <div className="flex items-start gap-1.5 text-[#A1A1AA] text-sm min-w-0 max-w-full">
                          <svg
                            className="w-4 h-4 text-[#9CC88D] shrink-0 mt-0.5"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            strokeWidth={2}
                      ...
                      {user?.faculty && (
                        <div className="flex items-start gap-1.5 text-[#A1A1AA] text-sm min-w-0 max-w-full">
                          <span className="text-[#9CC88D] shrink-0">
                            🎓
                          </span>
                          <span className="break-words">
                            {user.faculty}
                ...
                <div className="w-full md:w-auto shrink-0 mt-2 md:mt-0">
                  <Link 
                    to="/edit-profile"
                    className="w-full md:w-auto px-6 py-2.5 bg-[#27272A] border border-[#3F3F46] rounded-xl text-[#DDE4DD] text-base font-semibold hover:bg-[#3F3F46] transition-colors inline-block text-center shadow-sm">
                    Edit Profile
                  </Link>
                </div>
              </div>
            ...
            <div className="w-full bg-[#1A211D] border border-[#27272A]/50 shadow-sm rounded-2xl p-5 md:p-8 flex flex-col gap-6">
