...
export default function Messages() {
  ...
  const getOtherName = (room) => {
    ...
    if (user.id === room.initiator_id) return room.post_owner?.full_name || "Unknown";
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-10 flex flex-col items-center">
          <div className="w-full max-w-225 flex flex-col gap-6">
            <h2 className="text-[#E4E4E7] text-3xl font-bold leading-10">Messages</h2>

            <div className="relative">
              <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#86948A]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                <div className="w-16 h-16 rounded-full bg-[#18181B] border border-[#27272A] flex items-center justify-center">
                  <svg className="w-8 h-8 text-[#3C4A42]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                  </svg>
                </div>
                <div className="text-center">
                  <p className="text-[#86948A] text-base font-medium">{searchQuery ? "No conversations found" : "No messages yet"}</p>
                  <p className="text-[#3C4A42] text-sm mt-1">{searchQuery ? "Try a different search" : 'Start a conversation via an item\'s detail page'}</p>
                </div>
              </div>
            ) : (
                    ...
                    <div key={room.room_id} onClick={() => navigate(`/messages/${room.room_id}`)}
                      ...
                      <div className="shrink-0 flex items-center self-center">
                        {room.posts?.item_image ? (
                          <img src={room.posts.item_image} alt={room.posts.caption} className="w-14 h-14 rounded-lg object-cover shadow-sm" />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0 py-1 flex flex-col justify-center">
                        <span className="text-sm font-medium leading-5 tracking-tight truncate text-[#BBCABF]">{otherName}</span>
                        {room.posts?.caption && (
                          <div className="flex items-center gap-1.5 mb-1">
                            <svg className="w-3 h-3 shrink-0 text-[#71717A]" fill="currentColor" viewBox="0 0 20 20">
                              <path fillRule="evenodd" d="M17.707 9.293a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7A.997.997 0 012 10V5a3 3 0 013-3h5c.256 0 .512.098.707.293l7 7zM5 6a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
                            </svg>
                            <span className="text-xs font-semibold text-[#71717A] truncate">{room.posts.caption}</span>
                          </div>
                        )}
                        <p className="text-base leading-6 truncate text-[#71717A]">Tap to open conversation</p>
                      </div>
                      <div className="shrink-0 flex flex-col items-center gap-6 py-1">
                        <span className="text-xs font-semibold text-[#71717A] whitespace-nowrap">{formatTime(room.created_at)}</span>
                      </div>
                    </div>
                  );
                })}
