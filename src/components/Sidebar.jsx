...
const NAV_LINKS = [
  ...
  { to: "/admin", key: "admin", label: "Admin", icon: "M9 17v-2a4 4 0 014-4h4m0 0V7m0 4h-4m-6 8H5a2 2 0 01-2-2V7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v2" },
  { to: "/lost-items", key: "lost-items", label: "Lost Items", icon: "M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" },
  { to: "/found-items", key: "found-items", label: "Found Items", icon: "M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" },
  { to: "/my-reports", key: "my-reports", label: "My Reports", icon: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
  { to: "/messages", key: "messages", label: "Messages", icon: "M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" },
  { to: "/watchlist", key: "watchlist", label: "Watchlist", icon: "M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" },
];
...
function Sidebar({ activePage = "", hideMobileNav = false }) {
  ...
  const checkUnreadMessages = useCallback(async () => {
    try {
      ...
      const hasUnread = notifications.some((n) => !n.is_read && n.type === "message");
  ...
  useEffect(() => {
    checkUnreadMessages();
    const intervalId = window.setInterval(checkUnreadMessages, 30000);
      ...
      <aside className="hidden md:flex flex-col w-[256px] bg-[#0E1511] border-r border-[#3C4A42]/30 shrink-0 z-20 shadow-[4px_0px_24px_rgba(0,0,0,0.18)]">
        <div className="p-6 border-b border-[#3C4A42]/30">
          <Link to="/profile" className="flex items-center gap-3 group cursor-pointer">
            {avatarUrl ? (
              <img src={avatarUrl} alt={displayName} className="w-10 h-10 rounded-full border-2 border-[#9CC88D] object-cover group-hover:border-[#8bb47d] transition-colors" />
            ) : (
              <div className="relative w-10 h-10 rounded-full border-2 border-[#9CC88D] bg-[#164A41] flex items-center justify-center overflow-hidden group-hover:border-[#8bb47d] transition-colors">
                <svg className="w-7 h-7 text-[#9CC88D] absolute -bottom-0.75" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <p className="text-[#86948A] text-[13px] font-medium leading-tight mt-1 group-hover:text-[#BBCABF] transition-colors">
                {role === "admin" ? "Admin" : role === "staff" ? "Staff Portal" : "Student Portal"}
              </p>
            </div>
          </Link>
        </div>
            ...
            return (
              <Link key={link.to} to={link.to}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors relative group ${
                  isActive ? "bg-[#1A211D] text-[#9CC88D] border-r-4 border-[#9CC88D]" : "text-[#86948A] hover:bg-[#1A211D] hover:text-[#DDE4DD]"
                }`}>
        ...
        <div className="p-4 border-t border-[#3C4A42]/30 space-y-1">
          <Link to="/help" className="flex items-center gap-3 px-4 py-3 text-[#86948A] hover:text-[#DDE4DD] hover:bg-[#1A211D] rounded-lg transition-colors">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Help Center
          </Link>
          <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-4 py-3 text-[#86948A] hover:text-[#DDE4DD] hover:bg-[#1A211D] rounded-lg transition-colors">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
